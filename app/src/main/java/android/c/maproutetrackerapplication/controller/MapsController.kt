package android.c.maproutetrackerapplication.controller

import android.c.maproutetrackerapplication.factory.MapsFactory
import android.content.Context
import com.google.android.gms.maps.CameraUpdateFactory
import com.google.android.gms.maps.GoogleMap
import android.c.maproutetrackerapplication.model.others.Spot
...
class MapsController(context: Context, googleMap: GoogleMap)
{
    ...
    private val mTimeSquare = LatLng(12.277110,76.620970)
    ...
    fun setCustomMarker() {
        ...
        val markerOptions: MarkerOptions = MarkerOptions().position(mTimeSquare).title(mContext.getString(R.string.source)).snippet(mContext.getString(R.string.i_am_snippet)).icon(blackMarkerIcon)
    ...
    fun setMarkersAndRoute(route: Route) {
        ...
        val startMarkerOptions: MarkerOptions = MarkerOptions().position(startLatLng).title(route.startName).icon(BitmapDescriptorFactory.fromBitmap(
            MapsFactory.drawMarker(mContext, "Village")
        ))
        ...
        val endMarkerOptions: MarkerOptions = MarkerOptions().position(endLatLng).title(route.endName).icon(BitmapDescriptorFactory.fromBitmap(
            MapsFactory.drawMarker(mContext, "Market")
        ))
